...
class TimbreZGenerator(nn.Module):
    def __init__(self, timbre_emb_dim, fl_emb):
        super().__init__()
        self.linear = linear_stack(timbre_emb_dim, timbre_emb_dim)
        self.linear_2 = linear_stack(timbre_emb_dim, timbre_emb_dim)
        ...
        self.affine_block_2 = TimbreAffineBlcok(timbre_emb_dim, fl_emb)
    
    def forward(self, timbre_emb, l_emb):
        out = self.linear(timbre_emb)
        fusion = self.fusion_block(out, l_emb)
        affine = self.affine_block(out, l_emb)
        out = out + fusion + affine
        out = self.linear_2(out)
        fusion = self.fusion_block_2(out, l_emb)
        affine = self.affine_block_2(out, l_emb)
        out = out + fusion + affine
        return out
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.timbre_linear = linear_stack(timbre_emb_size, timbre_emb_size)
    ...
    def forward(self, f0, loudness, energy, timbre_emb):
        ...
        timbre_z = self.timbre_z_generator(timbre_emb, loudness)
        out_t_linear = self.timbre_linear(timbre_z)
        cat_input = torch.cat(
            [
                out_f0_linear,
                out_l_linear,
