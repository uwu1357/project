...
class CondMLP(nn.Module):
    def __init__(self, in_size, hidden_size):
        ...
        self.linear_4 = self.linear_stack(hidden_size, hidden_size)
...
class TimbreResidualMixer(nn.Module):
    def __init__(self, timbre_emb_dim, residual_emb_dim):
        super().__init__()
        self.linear_1 = self.linear_stack(timbre_emb_dim, timbre_emb_dim)
        self.linear_2 = self.linear_stack(timbre_emb_dim, timbre_emb_dim)
        self.linear_3 = self.linear_stack(timbre_emb_dim, timbre_emb_dim)
        self.residual_DFB_1 = DFBlock(timbre_emb_dim, residual_emb_dim, affine_dim=timbre_emb_dim, out_layer_mlp=True)
        self.residual_DFB_2 = DFBlock(timbre_emb_dim, residual_emb_dim, affine_dim=timbre_emb_dim, out_layer_mlp=True)
        self.residual_DFB_3 = DFBlock(timbre_emb_dim, residual_emb_dim, affine_dim=timbre_emb_dim, out_layer_mlp=True)
        self.residual_DFB_4 = DFBlock(timbre_emb_dim, residual_emb_dim, affine_dim=timbre_emb_dim, out_layer_mlp=True)
    
    def forward(self, timbre_emb, residual_emb): 
        timbre_emb = self.linear_1(timbre_emb)
        x = self.residual_DFB_1(timbre_emb, residual_emb)
        x = self.residual_DFB_2(x, residual_emb)
        timbre_emb = x + timbre_emb
        timbre_emb = self.linear_2(timbre_emb)
        x = self.residual_DFB_3(timbre_emb, residual_emb)
        x = self.residual_DFB_4(x, residual_emb)
        timbre_emb = x + timbre_emb
        timbre_emb = self.linear_3(timbre_emb)
        return timbre_emb
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.t_r_mixer = TimbreResidualMixer(timbre_emb_size, z_units)
    ...
    def forward(self, f0, loudness, z, timbre_emb):
        ...
        out_t_r_mixer = self.t_r_mixer(timbre_emb, out_z_mlp)

        out_f0_mlp = self.f0_self_att(out_f0_mlp, out_f0_mlp)
        out_l_mlp = self.l_self_att(out_l_mlp, out_l_mlp)
        out_t_r_mixer = self.timbre_self_att(out_t_r_mixer, out_t_r_mixer)
