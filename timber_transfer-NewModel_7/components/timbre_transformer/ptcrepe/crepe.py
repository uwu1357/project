...
class CREPE(nn.Module):
    ...
    def get_activation(self, audio, sr, center=True, step_size=10, batch_size=128):
        ...
        for i in range(batch):        
            batch_frames.append(get_frame(audio[i], step_size, center).unsqueeze(0))
        
        frames = torch.cat(batch_frames, dim=0)
        frames = frames.view(-1, 1024) 
        activation_stack = []
        ...
        activation = activation.view(batch, -1, 360)
        return activation
    ...
    def predict(self, audio, sr, viterbi=False, center=True, step_size=10, batch_size=128):
        ...
        frequency = to_freq(activation.view(-1, 360), viterbi=viterbi).view(batch, -1)
        confidence = activation.max(dim=2)[0]
